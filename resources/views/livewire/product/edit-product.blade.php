        ...
        <form wire:target="submit" wire:submit.prevent="submit">
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    @if ($product->verified_at)
                        <div class="fw-bold text-success mb-3">
                            Your domain has been verified at
                            {{ carbon($product->verified_at)->format('d M Y g:i A') }}
                        </div>
                    @else
                        <div>This product is not yet verified</div>
                        <div class="mt-2">
                            <a href="{{ route('product.verify', ['slug' => $product->slug]) }}"
                                class="btn btn-sm btn-outline-success rounded-pill">Verify now</a>
                        </div>
                    @endif
                </div>
                <div class="mb-3">
                ...
                <button type="button" onclick="confirm('Are you sure?') || event.stopImmediatePropagation()"
                    wire:click="deleteProduct" wire:target="deleteProduct" wire:loading.attr="disabled"
                    aria-label="Delete" class="btn btn-outline-danger rounded-pill">

<div class="card-footer text-muted">
    <img loading=lazy class="avatar-20 rounded-circle" src="{{ Helper::getCDNImage($meetup->user->avatar, 50) }}" alt="{{ $meetup->user->username }}'s avatar" />
    @foreach($meetup->subscribers->take(5) as $user)
    <img loading=lazy class="avatar-20 rounded-circle" src="{{ Helper::getCDNImage($user->avatar, 50) }}" alt="{{ $user->username }}'s avatar" />
    @endforeach
    @auth

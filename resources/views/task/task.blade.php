        ...
        <div class="col-sm">
            <div class="card mb-4">
                ...
                <div class="card-body d-flex align-items-center">
                    ...
                    >
                        <img loading=lazy class="rounded-circle avatar-40 mt-1" src="{{ Helper::getCDNImage($task->user->avatar) }}" alt="{{ $task->user->username }}'s avatar" />
                    </a>
                    <span class="ms-3">
                        <a
                            href="{{ route('user.done', ['username' => $task->user->username]) }}"
            ...
            <div class="card mb-4">
                ...
                <div class="card-body d-flex align-items-center">
                    ...
                    >
                        <img loading=lazy class="rounded avatar-40 mt-1" src="{{ Helper::getCDNImage(\App\Models\Product::find($task->product_id)->avatar) }}" />
                    </a>
                    <span class="ms-3">
                        <a
                            href="{{ route('product.done', ['slug' => \App\Models\Product::find($task->product_id)->slug]) }}"
            ...
            <div class="card mb-4">
                ...
                <div class="card-body align-items-center pb-2">
                    @foreach ($task->comments->groupBy('user_id') as $comment)
                        ...
                        >
                            <img loading=lazy class="rounded-circle avatar-30 mb-2" src="{{ Helper::getCDNImage($comment[0]->user->avatar) }}" alt="{{ $comment[0]->user->username }}'s avatar" />
                        </a>
                    @endforeach
                </div>
            </div>

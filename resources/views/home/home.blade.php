                ...
                <div class="card-body">
                    @foreach ($recent_questions as $question)
                        <div class="{{ $loop->index === count($recent_questions) - 1 ? '' : 'mb-2' }} {{ $question->patronOnly ? 'bg-patron recent-questions' : '' }}">
                            ...
                            >
                                <img loading=lazy class="rounded-circle avatar-30" src="{{ Helper::getCDNImage($question->user->avatar) }}" alt="{{ $question->user->username }}'s avatar" />
                            </a>
                            ...
                            <span class="avatar-stack ms-1">
                                ...
                                <img loading=lazy class="replies-avatar rounded-circle {{ $loop->last ? 'me-0' : '' }}" src="{{ Helper::getCDNImage($answer[0]->user->avatar) }}" alt="{{ $answer[0]->user->username }}'s avatar" />
                                @endforeach
                            </span>
                        </div>
                    @endforeach
                    ...
                    <li class="list-group-item">
                        <div class="d-flex align-items-center">
                            <a href="{{ route('product.done', ['slug' => $product->slug]) }}">
                                <img loading=lazy class="rounded avatar-50 mt-1 ms-2" src="{{ Helper::getCDNImage($product->avatar) }}" alt="{{ $product->slug }}'s avatar" height="50" width="50" />
                            </a>
                            <span class="ms-3">
                            ...
                            <span class="ms-auto">
                                ...
                                <a
                                    ...
                                    data-id="{{ $user->id }}"
                                >
                                    <img loading=lazy class="rounded-circle avatar-30 mt-1 me-1" src="{{ Helper::getCDNImage($user->avatar) }}" alt="{{ $user->username }}'s avatar" />
                                </a>
                                @endforeach
                                <a
                                ...
                                >
                                    <img loading=lazy class="rounded-circle avatar-30 mt-1 me-0" src="{{ Helper::getCDNImage($product->owner->avatar) }}" alt="{{ $product->owner->username }}'s avatar" />
                                </a>
                            </span>
                        </div>
                    </li>
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <a href="{{ route('user.done', ['username' => Auth::user()->username]) }}">
                                <img loading=lazy class="rounded-circle avatar-50 mt-1" src="{{ Helper::getCDNImage(Auth::user()->avatar) }}" alt="{{ Auth::user()->username }}'s avatar" />
                            </a>
            ...
            {{-- <div class="card mb-4">
                ...
                <div class="d-flex align-items-center py-1 px-3">
                    <a href="{{ route('user.done', ['username' => $user->username]) }}">
                        <img loading=lazy class="rounded-circle avatar-40 mt-1" src="{{ Helper::getCDNImage($user->avatar) }}" alt="{{ $user->username }}'s avatar" />
                    </a>
                    <span class="ms-3">
            ...
            {{-- <div class="card mb-4">
                ...
                <div class="py-2 px-3">
                    <a
                        ...
                        data-id="{{ $product->id }}"
                    >
                        <img loading=lazy class="rounded avatar-30" src="{{ Helper::getCDNImage($product->avatar) }}" alt="{{ $product->slug }}'s avatar" height="50" width="50" />
                    </a>
                    <a
                        href="{{ route('product.done', ['slug' => $product->slug]) }}"
                ...
                <div class="pt-2 pb-2">
                    ...
                    <div class="py-2 px-3">
                        ...
                        <a
                            ...
                            data-id="{{ $user->id }}"
                        >
                            <img loading=lazy class="rounded-circle avatar-30 mt-1 ms-2" src="{{ Helper::getCDNImage($user->avatar) }}" height="50" width="50" alt="{{ $user->username }}'s avatar" />
                        </a>
                        <a
                            href="{{ route('user.done', ['username' => $user->username]) }}"
